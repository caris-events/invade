...
- **check_vocab**: ⭐ **檢查詞彙是否為侵略性詞彙** - 在輸出中文前使用此工具檢查
  - 參數：word（要檢查的詞彙）
  - 用途：LLM 在生成中文內容前，先檢查詞彙是否在資料庫中，避免使用侵略性詞彙
  - 回傳：如果詞彙在資料庫中，會提供警告和建議的替代詞彙
...
3. **檢查詞彙（推薦 LLM 使用）**：
   ...
   - "我想使用『硬件』這個詞，有問題嗎？"
   - "『資料庫』可以使用嗎？"
   - **使用場景**：LLM 在生成中文回覆前，先用此工具檢查詞彙，避免使用支語
